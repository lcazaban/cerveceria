...
namespace Cerveceria.Web.Controllers
{
    public class LoginController : Controller
    {
        ...
        [HttpPost]
        public JsonResult LoginComprar(string user, string pass)
        {
            if (Autenticar(user, pass) && !string.IsNullOrEmpty(user) && !string.IsNullOrEmpty(pass))
            {
                var usuario = _dbContext.Usuarios
                    .Where(a => a.Mail.Equals(user)).FirstOrDefault();

                var identity = new ClaimsIdentity(CookieAuthenticationDefaults.AuthenticationScheme, ClaimTypes.Name, ClaimTypes.Sid);
                identity.AddClaim(new Claim(ClaimTypes.Name, usuario.Nombre));
                identity.AddClaim(new Claim(ClaimTypes.Sid, usuario.Id));
                var principal = new ClaimsPrincipal(identity);
                HttpContext.SignInAsync(CookieAuthenticationDefaults.AuthenticationScheme, principal,
                    new AuthenticationProperties { ExpiresUtc = DateTime.Now.AddDays(1), IsPersistent = true });

                return Json(JsonConvert.SerializeObject(new { statusLogin = true , name = usuario.Nombre}));

            }
            else
            {
                return Json(JsonConvert.SerializeObject(new { statusLogin = false }));
            }
        }
        [HttpGet]
        ...
        public IActionResult Registrar(RegistroViewModel modelRegistro)
        {
            ...
            if (!BuscarUsuario(modelRegistro.Email))
            {
                ...
                bool isSentMail = this._mailManager.SendMailSomee(modelRegistro.Email, asunto, body);

                if (!isSentMail)
                {
                    ModelState.AddModelError("registro", "Por favor, ingrese un mail valido.");
        ...
        public IActionResult RecuperarContraseña(RecuperarViewModel model)
        {
            ...
            if (BuscarUsuario(model.Email))
            {
                ...
                this._mailManager.SendMailSomee(model.Email, asunto, body);
        ...
        private Usuario GetUsuario(RegistroViewModel modelRegistro)
        {
            ...
            usuario.Telefono = modelRegistro.Telefono;
